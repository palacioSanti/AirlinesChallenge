                    ...
                    @foreach ($airlines as $airline)
                        <tr id="airline-{{ $airline->id }}">
                            ...
                            <td class="py-2 px-4 border-b">
                                <button onclick="editAirline({{ $airline->id }})"
                                    class="bg-yellow-500 text-white p-2 rounded mr-2">Edit</button>
    ...
    <script>
        ...
        async function editAirline(id) {
            ...
            if (response.ok) {
                ...
                const airline = airlineJson.airline;
                const row = document.getElementById(`airline-${id}`);
                row.querySelector('td:nth-child(2)').innerText = airline.name;
                row.querySelector('td:nth-child(3)').innerText = airline.description;
            } else {
                console.error('Error editing airline');
            }
